  ...
  try {
    ...
    if (health === 'failed') {
      return res.status(500).json({
        health: 'failed', service: 'auth-service',
    ...
    if (health === 'degraded') {
      return res.json({
        health: 'degraded', service: 'auth-service',
        timestamp: new Date().toISOString(),
    ...
    res.json({ health: 'healthy', service: 'auth-service', timestamp: new Date().toISOString(), dependencies: serviceState.dependencyHealth });
  } catch (error) {
    res.status(500).json({ health: 'failed', service: 'auth-service', error: error.message, timestamp: new Date().toISOString() });
  }
});
